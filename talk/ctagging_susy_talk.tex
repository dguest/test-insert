...
\begin{frame}
  ...
  \begin{center}
    \graphic[0.8]{figures/external/standard-model.pdf}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item If natural SUSY exists, we could see it at the LHC
      ...
      %% \end{itemize}
    \item Focus on simplified models: only one or two sparticles accessible (e.g. $\tilde{t}$ + $\neut$)
      \begin{itemize}
        \item makes reinterpretation easier
        \item run over a ``grid'' of mass points
      \end{itemize}
    \item In our case, assume $R$ parity
      \begin{itemize}
      \item sparticles are pair-produced
        \item decay to $\neut$, the lightest SUSY particle (LSP)
      \end{itemize}
  \end{itemize}
  \begin{center}
...
\begin{frame}
  \frametitle{Comparison with Non--$c$-tagged searches%% $\tilde{q} \to \mathrm{0L} + \text{jets}$ search
  }
  \begin{columns}
    \begin{column}{0.6\textwidth}
