...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item If we saw it at the LHC, you'd have heard about it
      \begin{itemize}
      \item Starting to look rather ``un-natural''
...
\begin{frame}
  \frametitle{Why charm tag: $\sttoc$}
  \begin{columns}
    \begin{column}{0.6\textwidth}
  \begin{center}
...
\begin{frame}
  \frametitle{Why charm tag: $\sctoc$}
  \begin{center}
...
\begin{frame}
  ...
  \begin{itemize}
  \item ``biggest experiment ever'' (it's 27 km \emph{long}, is that big?)
  \item ``highest energy ever'' (pretty high energy $~\sim 10\,\tev$\ldots)
...
\begin{frame}
  \frametitle{Tracking and Vertexing}
  \includegraphics[width=\textwidth]{figures/external/pileup-vertices.png}
  \begin{itemize}
  \item In 2012, $\approx 20$ vertices per bunch crossing
  ...
  \item Displaced vertices can travel mm--cm
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item Standard ``black box'' supervised classifier
...
\begin{frame}
  ...
  \item The idea: a neural net can approximate any continuous function
    ...
    \item Can also be written recursively
      \begin{equation*}
        \vect{z}_{i+1} = f(A_{i} \vect{z}_{i} + \vect{b}_{i}), \qquad f = \mathbb{1} \quad \text{if}\quad i = N
      \end{equation*}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item Calculate error for $\vect{y}$, $\xi = (\vect{t} - \vect{y})$
...
\begin{frame}
  ...
  \begin{itemize}
  \item Reminder: a neural net can approximate any continuous function
    \begin{equation*}
          \vect{y} = \mathcal{N}(\vect{x}) = A_N f(A_{N -1} \cdots f(A_1 \vect{x} + \vect{b}_1) \cdots + \vect{b}_{N-1} ) + \vect{b}_N
    \end{equation*}
  \item $A_i$ and $\vect{b}_i$ are initially random
  \item Training relies on backpropagation, 
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Particle Identification}
  \begin{itemize}
  \item We only directly reconstruct 7 ``particles''
  \end{itemize}
  \begin{columns}
    \begin{column}{0.35\textwidth}
      ...
      \item \textcolor{orange}{\textbf{charm}} used to be ``light''
      \end{itemize}
    \end{column}
