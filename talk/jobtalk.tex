...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.7\textwidth}
      ...
      \item For each target $\vect{t}$ and input $\vect{x}$
      \item Calculate error for $\vect{y}$, $\vect{\xi}_{N+1} = (\vect{t} - \vect{y})$
      \item Backpropagate errors $\xi_i = (A_i^{\intercal} \xi_{(i+1)}) \circ f'(\vect{z}_i)$
      \item Update $A_i \to A - \gamma \xi_{(i+1)}g(\vect{z}_i)^{\intercal}$, $\vect{b}_i \to \vect{b} - \gamma \xi_{(i+1)}$
...
\begin{frame}
  \frametitle{Overview: What $c$-tagging gives us}
  \begin{itemize}
