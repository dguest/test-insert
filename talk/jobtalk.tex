...
\begin{frame}[fragile=singleslide]
  ...
  \item Solution: add an \textcolor{orange}{ISR boosted} signal region
    ...
    \item Even better: simplified SUSY models mean $\sigma_{\tilde{b}\tilde{b}} \approx \sigma_{\tilde{c}\tilde{c}}$
...
int/figures/stackplots/dans/preselection/jetmet_dphi.pdf}
  ...
  \end{columns}
  \begin{itemize}
  \item Requiring $\Delta R(j_{\{1,2,3\}},\met) > 0.4$ removes almost all these events
  \item Remaining are mostly light flavor $\to$ removed by $c$-tagging
...
\begin{frame}
  \frametitle{Backgrounds: $t\bar{t}$ + jets}
  \begin{columns}
    \begin{column}{0.5\textwidth}
    \feynincstd{wjets}
    \end{column}
    \begin{column}{0.5\textwidth}
    \feynincstd{zjets} \\[1cm]
    \feynincstd{ttbar}
    \end{column}
  \end{columns}
\end{frame}
