...
Contrasting from the rigid simplicity of the gauge bosons, the Higgs boson mass and couplings are still rather \emph{ad hoc}. From \cref{eq:higgs}, the Higgs mass is given by $m_{H} = \lambda v^2$, where $\lambda$ is a free parameter which appears nowhere else in the standard model. In addition, since the $H^3$ and $H^4$ couplings remain untested the shape of the Higgs potential is unconfirmed beyond the first order. Many more \emph{ad hoc} parameters enter into the SM when mass terms are added to the fermions in the form of Yukawa couplings.

The Yukawa interactions couple the Higgs field to fermions, as described in the final component of the SM lagrangian
\begin{equation}
  \lagr_{\mathrm{Yuk}} =
  - \Gamma^u_{mn} \bar{q}_{m \mathrm{L}} \tilde{\phi} u_{n\mathrm{R}}
  - \Gamma^d_{mn} \bar{q}_{m \mathrm{L}} \phi d_{n\mathrm{R}}
  - \Gamma^e_{mn} \bar{\ell}_{m \mathrm{L}} \phi e_{n\mathrm{R}}
\end{equation}
