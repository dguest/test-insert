...
Given that cross-section is directly related to production rate, it's no surprise that particle discoveries over the last century move roughly from higher to lower cross-sections; the bottom quark was discovered before the $W$ and $Z$ bosons~\cite{b-prl}, which were discovered before the top quark and the Higgs boson.\footnote{Note that the progression from higher-to-lower $pp$ cross-section is a gross oversimplification of history. The $W$ and $Z$ were, for example, discovered at an $e^+ e^-$ collider, where the $pp$ cross-section is clearly unimportant.}
...
Momentum along the beam axis, by contrast, can carry over from the incoming particle momentum.
...
Complementing $\pt$, which describes the outward momentum of particles, the angle with respect to the beam axis is described by pseudorapidity $\eta$, defined as $\eta \equiv - \ln [ \tan(\theta / 2) ]$, where $\theta$ is the angle relative to the beam axis.
...
Prompt quarks, as color-charged objects, form hadronic jets of non-isolated charged and neutral particles.
...
%% \end{itemize}
%% \subsection{Neutrinos (and Neutralinos)}
%% the known unknowns
%% \begin{itemize}
