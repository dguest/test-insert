...
First, JetFitterCharm's $b$-vs-light discrimination is is inferior to that from MV1, the baseline $b$-tagger in run 1. This is surprising given that both are neural nets with roughly the same inputs.
Secondly, the neural net training isn't reproducible; retraining on exactly the same dataset may produce performance that's better or worse by several percent.
