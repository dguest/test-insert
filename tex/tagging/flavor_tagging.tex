...
An $N + 1$ layer neural net can be represented as stack of linear transformations $A_{i}$ interleaved with a nonlinear activation function $f$~\cite{luke-thesis}:
\begin{equation}
...
\item The error is backpropagated to the previous (hidden) layer using the $A_i$ matrix.
\end{enumerate}

\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{figures/external/uRejRoc.pdf}
