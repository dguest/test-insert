...
An $N$ layer neural net can be represented as stack of linear transformations represented as matrices $A_{i}$ interleaved with a nonlinear activation function $f$:
\begin{equation}
...
Thus given some number of inputs and outputs, the neural net is still left with $N - 2$ free dimensions---one for each hidden layer.
